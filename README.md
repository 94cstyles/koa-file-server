# 静态文件服务 for koa2
静态文件服务，除却基本文件服务，增加了图片处理，请求合并，文本文件gzip压缩等功能。因为使用了es6以及es7语法，所以需要 [babel](https://babeljs.io/) 来运行。
...
import Koa from 'koa';
import serve from 'koa2-file-server';
...
### 图片处理重心参数表
**注意**：该选项只影响其后的[裁剪操作参数表](#裁剪操作参数表)，即裁剪操作以gravity为原点开始偏移后，进行裁剪操作。
