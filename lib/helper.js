...
const debug = _debug('koa-file-server:helper');
...
const helper = {
    ...
    validateParam: function (name, value) {
        let rules = this.paramValueRules[name];
    ...
    parsePath: function (p, type) {
        return fs.stat(p).then(function (stats) {
            ...
            if (err.code === 'ENOENT' || err.code === 'ENAMETOOLONG' || err.code === 'ENOTDIR')  return null;
            throw err;
        });
    },
    /**
    ...
    isFile: async function (filePath) {
        return await this.parsePath(filePath, 'isFile');
    },
    /**
     * 判断该路径是否为文件夹
     * @param dirPath 目录路径
    ...
    isDirectory: async function (dirPath) {
        return await this.parsePath(dirPath, 'isDirectory');
    },
    /**
     * 判断文件目录是否存在 不存在则创建
     * @param root 起始目录
     ...
     */
    mk: async function (root, filePath) {
        let dirPath = root;
        let fileDir = path.parse(filePath).dir;
        debug('判断目标文件目录是否存在,不存在则创建' + fileDir);
        //先验证文件目录是否存在
        if (!fs.existsSync(fileDir)) {
            let dirNames = fileDir.replace(root, '').split(path.sep);
