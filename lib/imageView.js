...
const imageView = {
  ...
  create: async function (root, sourcePath, filePath, options) {
    ...
    if (helper.mk(root, filePath)) {
      ...
      const size = await file.sizeSync()
      ...
      // 输出文件
      const err = await file.writeSync(filePath)
      const stats = err ? null : await helper.isFile(filePath)
  ...
  getThumbnailParam: function (paramStr, size) {
    let width, height, option, result
    ...
    if (result = paramStr.match(helper.paramValueRules.thumbnail[0])) {
      ...
      // 文档1 2 3
      const ratio = Math.min(Number(result[1] / 100), 10)
