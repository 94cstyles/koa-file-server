...
const debug = require('debug')('koa-file-server:parseImage')
 ...
 * @returns {object}
 */
module.exports = function parseImage (root, sourceFile, optionsStr, cacheDirName, webp) {
  const options = []
  const optionsMatch = optionsStr.match(/([^&=]+)=?([^&]*)/g)
  if (optionsMatch) {
    for (const str of optionsMatch) {
      // 是否有图片处理
      if (/^imageView\/.{1,}$/.test(str)) {
        let key, value, error
        const optionsArr = str.replace('imageView/', '').split('/')
