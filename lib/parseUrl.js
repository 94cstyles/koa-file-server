...
const debug = _debug('koa-file-server:parseUrl');
 ...
 * @returns {object}
 */
export default function parseUrl(root, options, url, accept) {
    debug('解析请求:' + url);

    //是否包含文件合并标识符
    if (url.indexOf(options.identifier) === -1) {
        let parts = url.match(/^(.*?)\?(.*?)$/) || [url, url, '']; //拆分url
        let file = path.parse(parts[1]);
        ...
        if (helper.sourceImgExt.test(file.ext)) {
            ...
            let sourcePath = path.join(root, parts[1]);
            return {
                status: 200,
                body: {
                    filePath: sourcePath,
        ...
        let files, filePath, parts, ext, _ext;

        parts = url.split(options.identifier); //拆分url
