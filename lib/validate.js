...
const debug = require('debug')('koa-file-server:validate')
 ...
 * @returns {object}
 */
module.exports = async function validate (root, request) {
  let stats
    ...
    if (!(stats = await helper.isFile(request.body.filePath))) {
      ...
      if (request.body.imageView && request.body.filePath !== request.body.sourcePath) {
        ...
        if (stats) {
          ...
          // 使用gm进行处理文件
          const result = await imageView.create(root, request.body.sourcePath, request.body.filePath, request.body.imageView)
          return result
        } else {
